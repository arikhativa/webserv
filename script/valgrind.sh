...
run_with_valgrind()
{
	valgrind --leak-check=full --show-leak-kinds=all --errors-for-leak-kinds=all --track-origins=yes --log-file=$VALGRIND_OUTPUT $ROOT_DIR/$TEST_EXEC > $ROOT_DIR/$TEST_RES
}
